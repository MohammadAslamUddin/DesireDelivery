...
namespace DesireDelivery.Gateway.Owner
{
    public class FoodsGateway : CommonGateway
    {
        ...
        public List<Foods> GetFoodBySearching(string stri)
        {
            if (stri.Equals(""))
            {
                Query = "SELECT food.food_id,food.food_name,food.food_image,food.food_price,Restaurant.restaurant_name FROM Food full join Restaurant on Food.restaurant_id = Restaurant.restaurant_id;";
            }
            else
            {
